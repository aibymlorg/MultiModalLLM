...
import os
import requests
from utils import load_env, llama32, disp_image, check_ollama_available, get_available_ollama_models
warnings.filterwarnings('ignore')
load_env()

# Check if Ollama is available
OLLAMA_AVAILABLE = check_ollama_available()
OLLAMA_MODELS = get_available_ollama_models() if OLLAMA_AVAILABLE else []

# Define preset Ollama models to look for
PRESET_OLLAMA_MODELS = ["llama3.2-vision:90b", "mistral-small3.1:24b"]

# Filter preset models that are actually available
AVAILABLE_PRESET_MODELS = [model for model in PRESET_OLLAMA_MODELS if model in OLLAMA_MODELS]

def kid_draw_analysis(image_path, initial_question, model_selection):
    ...
    if model_selection != "Together.ai (Llama-3.2-11B)":
        if "llama3.2-vision:90b" in model_selection:
            use_ollama = True
            ollama_model = "llama3.2-vision:90b"
        elif "mistral-small3.1:24b" in model_selection:
            use_ollama = True
            ollama_model = "mistral-small3.1:24b"
    
    # First analysis
    result = process_image_for_llama(image_path, initial_question, use_ollama, ollama_model)
    return result

def follow_up_analysis(image_path, initial_response, followup_question, model_selection):
    ...
    final_result = llama32(messages, use_ollama=use_ollama, ollama_model=ollama_model)
...
def process_image_for_llama(image_path, prompt, use_ollama=False, ollama_model=None):
    ...
    return llama32(messages, use_ollama=use_ollama, ollama_model=ollama_model)
...
def get_model_choices():
    # Always include Together.ai as an option
    choices = ["Together.ai (Llama-3.2-11B)"]
    
    # Add available Ollama models
    if OLLAMA_AVAILABLE:
        for model in AVAILABLE_PRESET_MODELS:
            choices.append(f"Ollama - {model} (local)")
        
        # If none of the preset models are found but Ollama is available
        if not AVAILABLE_PRESET_MODELS:
            choices.append("Ollama - No compatible vision models found")
    
    # If Ollama is not available
    if not OLLAMA_AVAILABLE:
        choices.append("Ollama - Not available (install locally to use)")
...
with gr.Blocks(
    ...
    with gr.Group(elem_id="app-container"):
        ...
        with gr.Accordion("Instructions", open=False):
            ...
            3. Choose your model (Together.ai or local Ollama)
        ...
        model_dropdown = gr.Dropdown(
            ...
            value="Together.ai (Llama-3.2-11B)",  # Default to Together.ai
            info="Local Ollama models require installation on your computer"
        )

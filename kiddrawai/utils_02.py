...
def llama32(messages, model_size=11, use_ollama=False, ollama_model=None):
    """
    Process messages with either Together.ai API or local Ollama model
    
    Args:
        messages: List of message objects with role and content
        model_size: Size of the Together.ai model (default: 11B)
        use_ollama: Whether to use local Ollama model instead of Together.ai API
        ollama_model: Specific Ollama model to use (e.g., "llama3.2-vision:90b" or "mistral-small3.1:24b")
    
    Returns:
        Generated response text
    """
    if use_ollama and ollama_model:
        return ollama_process(messages, ollama_model)
    else:
        # Use Together.ai API
...
def ollama_process(messages, model_name):
    """
    Process messages using a local Ollama model
    ...
    # Ollama API endpoint (assuming local installation)
    url = "http://localhost:11434/api/chat"
    
    # Convert image URLs to base64 if present
            ...
            for i, content_item in enumerate(message["content"]):
                if isinstance(content_item, dict) and content_item.get("type") == "image_url":
                    ...
                    if image_url.startswith("data:image/jpeg;base64,"):
                        # Image is already in base64 format, extract it
                        base64_data = image_url.replace("data:image/jpeg;base64,", "")
                        # Replace the image_url with the base64 data
                        message["content"][i] = {
                            "type": "image", 
                            "data": base64_data
                        }
...
def llama31(prompt_or_messages, model_size=8, temperature=0, raw=False, debug=False, use_ollama=False, ollama_model=None):
    if use_ollama and ollama_model:
        # For Ollama, we'll only support the chat messages format, not raw prompts
        if not isinstance(prompt_or_messages, list):
            prompt_or_messages = [{"role": "user", "content": prompt_or_messages}]
        return ollama_process(prompt_or_messages, ollama_model)
...
def get_available_ollama_models():
    ...
    Returns:
        list: List of available model names, or empty list if Ollama is not available
    """
    try:
        response = requests.get("http://localhost:11434/api/tags")
        if response.status_code == 200:

...
with gr.Blocks(
    ...
    with gr.Group(elem_id="app-container"):
        ...
        # Speech input icons with text inputs
        with gr.Row():
            initial_q = gr.Textbox(
                label="☺️ Tell me about your drawing?",
                placeholder="Leave blank and I'll tell you what I see!",
                lines=2,
                scale=10   # Increased to make textbox take more space
            )
            mic_button = gr.Button("🎤", elem_classes="icon-button", scale=1)
        
        # The follow-up question moved before the "Show Me The Magic" button
        # and changed label to "Am I understanding your drawing?"
        with gr.Row():
    ...
    clear_button.click(
        fn=reset_interface,
        inputs=[],
        outputs=[image_input, initial_q, followup_q, art_output, speech_status]
    )
